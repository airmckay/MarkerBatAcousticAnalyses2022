...
# Outputs: 1. All bat activity aggregated to hour per site 
           AND all bat activity aggregated to hour per site - BINARY (1/0) 
           2. All guild and behavior bat activity aggregated to hour per site
           AND all bat activity aggregated to hour per site - BINARY (1/0) 
...
# write.csv(bats5, "nightlyaggregatedMarker2020_totalbats_zeroinserted_weather_binary.csv")
...
#write.csv(nbats3, "nightlyaggregatedMarker2020_totalbats_zeroinserted_behavior and guild.csv")
...
#write.csv(bats5,"nightlyaggregated_zeros_binary_behavior and guild.csv")
...
### MAKE SUPPLEMENTARY MATERIALS MAP### 
# levels(df3C$Site)
# 
# leftside <- df3C %>% filter(Site %in% c("P02",   "P04",   "P08",
#                                         "P09",  "P10",   "P11", "P14", "Met45" )) %>% droplevels()
# 
# rightside <- df3C %>% filter(Site %in% c("N02",   "N04",   "N08",
#                                         "N09",  "N10",   "N11", "N14", "Met90" )) %>% droplevels()
# 
# pl <- ggplot(data=leftside[leftside$Batpass_sum>0,]) +
#   geom_point(data=leftside, aes(x=night,y=guild),shape=19,color="grey") +
#   geom_point(aes(x=night,y=guild),shape=19,color="black") +
#   facet_wrap(~Site, nrow = 8)
# pl
# 
# pr <- ggplot(data=rightside[rightside$Batpass_sum>0,]) +
#   geom_point(data=rightside, aes(x=night,y=guild),shape=19,color="grey") +
#   geom_count(aes(x=night,y=guild),shape=19,color="black") +
#   facet_wrap(~Site, nrow = 8)
# pr
