...
# eventually, I can add a tidy way of setting up all file paths / input and output directories using the user information to guide those paths. Pierre has offered to help with that. 
...
# 19483 is the number of passes found in the dataset wrangled below - from taking the combined manual ids, cleaning the columns and parsing out multiple bat passes. The last data set we used ( COMPLETE_ZeroIncluded_ManualID_HourlyAggregated_Weather_behavior_guild_14072022_KE) had 19358 bat passes. These were perhaps lost when we merged with another data set (most likely the hourly weather data). Not catastrophic, but worth noting. 
...
## 
 file.name <- "Step1.tidy_raw_manualid_output"
# 
...
## This file S4U08483_20200808_002648.wav was tagged as a PAUR feeding buzz, but PAUR rarely show feeding buzzes. I revisited this file to check it and it is just a commuting PAUR that is perhaps approaching something, but we can't call that a feeding buzz. I need to change this to a commuting pass. 
...
## Had to make this correction unfortunately AFTER I had already prepared the whole dataset, so I am going to reuse the name manual_output3 to keep this simple... 
...
manual_output7 <- manual_output6 %>%
  ...
  mutate(behavior = case_when(
    ...
    TRUE ~ "Commuting"
  )) # Somehow, all of the BABA passes were defined as Feeding when this happened - all of these passes were just commuting so I need to fix that. 
...
#write.csv(manual_output15, file.path(output_today,"MarkerCleanedCombinedManualIDs.csv"))
