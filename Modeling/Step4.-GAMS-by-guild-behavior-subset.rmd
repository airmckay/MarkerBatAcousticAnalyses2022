...
Will try removing the wind and temperature interaction - at first just with the binary response variable to see how that looks. 

--------------------------------------------------------------------------------

# m6_LREf 
binary response 
removed the wind * temp intera
temperature is linear 

```{r}
m6_LREf <- gam(cbind(yes_batpass,no_batpass) ~
              s(Locality, bs = "re") +                      
              s(jnight, by = Habitat, bs = "gp", k = 35) +    
              Habitat +                                      
              temp_mean +                               
              s(wind_mean, bs = "ps", k = 5),                                
              data = LREf, method = "REML",                  
              family = binomial, select=TRUE)                
summary(m6_LREf) # 
...
#windows()
par(mfrow = c(2,2))
gam.check(m6_LREf, rep = 500)           
#look at the plots, but also the output to check if k needs to be adjusted
gam.check(m6_LREf)  
#rep=500 gives you a polygon on the QQ plot, which the observed values should lie within


overdispersion.m6_LREf <- sum( residuals(m6_LREf, "pearson")^2 ) / m6_LREf$df.residual
overdispersion.m6_LREf
# 0.984761 ... it is alright! 
...
bmp(file.path(output_today, "checking k dimensions m6_LREf bats_tot.jpeg"), width = 9, height = 6, units = "in", res = 350)

##windows()
plot(m6_LREf, pages=1,scheme=2,shade=TRUE,seWithMean = TRUE)

draw(m6_LREf, scales ="free")         #function 'draw' is from package 'gratia'
draw(m6_LREf, scales ="fixed")

dev.off() 

## Visualize the 3D GAM
bmp(file.path(output_today, "3D gam m6_LREf bats_tot.jpg"), width = 9, height = 6, units = "in", res = 350)
par(mfrow = c(1,2))

vis.gam(m6_LREf, view = c("temp_mean", "wind_mean"))    #Not a very elegant plot, you can find prettier solutions, but helps understand the interaction between tempp and wind
vis.gam(m6_LREf, view = c("wind_mean", "temp_mean")) 

dev.off()
```

## Recap 
Taking away the wind and temperature interaction did not make it easier to work with this model... eventually would like to make some predictions from this though. 
...
# 
# # library(itsadug)
# # pvisgam(m6_LREf, view = c("wind_mean", "temp_mean"))
# 
...
# 
# ct_int_pred1 <- predict(m6_LREf, newdata = ct_int_pred,
#                       se.fit = TRUE) %>%
#   as_tibble() %>%
#   cbind(ct_int_pred)
# 
...
# # # option 2 
# 
# fit <- predict.gam(m6_LREf, newdata = ct_int_pred, type = "response")
...
# mr2_SREf
Binary response, remove the wind and temp interaction
temperature is linear 

```{r}

m3_SREf <- gam(cbind(yes_batpass,no_batpass) ~
              s(Locality, bs = "re") + 
              Habitat + 
              s(jnight, by = Habitat, bs = "gp") +
              temp_mean +
...
Diagnostic plots show a flat line for jnight and habitat, not encouraging 
...
ct_int_pred1 <- predict(m3_SREf, newdata = ct_int_pred,
                      se.fit = TRUE) %>%
  as_tibble() %>%
  cbind(ct_int_pred)
...
fit <- predict.gam(m3_SREf, newdata = ct_int_pred, type = "response")
ct_int_pred2 <- data.frame(ct_int_pred, fit)
summary(ct_int_pred2)
...
## Recap
The k values are alright but the model is slightly under dispered 
