...
m15.5 <- gam(cbind(yes_batpass,no_batpass) ~
              s(Locality, bs = "re") +                       
              s(jnight, by = Habitat, bs = "gp", k = 35) +    
              Habitat +                                     
              temp_mean +                                
              s(wind_mean, k = 15), 
              data = bats_ground, method = "REML",                 
...
fit <- predict.gam(m15.5, newdata = ct_int_pred, type = "response")
...
ct_int_pred1 %>%
  ...
  geom_line(aes(colour=temp_mean)) 


## Predict when bat activity essentially stops
## Use a newdata set with low temp values 
...
### Plotting with Katrine 
ct_int_pred2 %>%
  ggplot(aes(x=jnight, y=fit)) +
  geom_line(aes(colour=temp_mean)) + facet_wrap(~Habitat)
## Do not plot wind or temperature with habitat because it is not in the model 


ct_int_pred2 %>%
  ggplot(aes(x=jnight, y=fit)) +
  geom_line() + facet_wrap(~Habitat)
## See Katrine's scripts to find a way to smooth the lines 
...
## Create an average turbine - natural , turbine , met activity 
## Plot bat activity over the season 
## line plot 
...
# Create a differnt prediction data set for predicting total bat activity across time between the two datasets. 

# plt2 <- ct_int_pred2 %>% dplyr::filter(wind_mean == 7, temp_mean == 14) %>%  ggplot( aes(x=jnight, y = fit, group = factor(Habitat))) +
#     geom_line() 
# plt2
...
m21 <- gam(Batpass_sum ~
              ...
              Habitat +                                     
              temp_mean +                                
              s(wind_mean, bs = "cc", k = 11) ,                      
              data = bats_tot, method = "REML",                 
              family = nb(), select=TRUE)                

summary(m21) 
## Tried this with Locality instead of Facility but the dispersion / k values were considerably worse off for it. 
```
...
dotchart(bats_tot$wind_mean)
dotchart(bats_tot$temp_mean)
...
One model - it can be the binary response or count response depending on which fits better
- Locality is the only random effect variable and it needs to be there
- Habiata and jnight
- temp_mean 
- wind_mean splined 

Two "new data"s and two different sets of prediction data sets to make three figures:
- one where average temperature and wind are single values representing the average 
Plotting x = jnight y = fitted bat pass facet wrap by habitat 

- several values for wind and temperature, plotting cutoffs 
Do not include Habitat in the plots - select one habitat to plot 
